...
async function main() {
  ...
  const oldRecords = await getDocs(productsRef);
  await Promise.all(oldRecords.docs.map((record) => deleteDoc(doc(db, 'product', record.id))));

  // Register default records
  console.log('Registering default records');
  await Promise.all(DEFAULT_VALUES.map((record) => addDoc(productsRef, record)));
